...
|---------|--------------|
| **2.7.11** | **เพิ่ม `case_status` ใน capture + แยกหน้า `/cancelled` สำหรับ "ยกเลิกเคลม"**: extension อ่าน radio `supervisor_summary` (close_case-inputEl / cancel_case-inputEl) ตอน user กดปุ่ม "ยืนยันการตรวจสอบ" — ค่าเข้า `case_status` = `"close"` (ปิดการตรวจสอบ) หรือ `"cancel"` (ยกเลิกเคลม) หรือ `null` (ไม่ได้เลือก). Server: เพิ่ม column `captures.case_status` (auto-migrate) + index, `GET /api/captures?status=close\|cancel` filter (close รวม legacy null), xlsx export ก็ filter ตามได้. UI: nav เพิ่ม "เคลมยกเลิก" ลิงก์ `/cancelled` (public) + `/admin/cancelled` (admin); หน้า `/cancelled` แสดงเฉพาะ status=cancel พร้อม column น้อยกว่า (ไม่มี ค่าบริการ/พาหนะ/รวม เพราะงานยกเลิกไม่มียอด); หน้า `/captures` เดิมแสดง close + legacy null ([content.js readCaseStatus](./content.js), [server/db.js](../server/db.js), [server/public/cancelled.html](../server/public/cancelled.html)) |
