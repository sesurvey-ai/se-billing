...
|---------|--------------|
| **2.3.0** | **เพิ่ม auto-calc % ของค่าเรียกร้อง**: ตรวจ `tab1_RECV_CLAIM` (numberfield ค่าเรียกร้อง) — ถ้า > 0 → ตั้ง `tab1_SUR_CLAIM` = 5% และ `tab1_INS_DAILY` = 10% ของ RECV; ถ้า ≤ 0 หรือว่าง → clear ทั้งสอง. ทำงานทุก mode (ไม่ขึ้นกับ `enabledProvinces` หรือ `AMPHUR_FEE_TABLE`). Trigger: native `change`/`input` listener + Ext component `change` (เผื่อ spinner buttons / setValue) + polling 500ms. Round 2 decimals |
